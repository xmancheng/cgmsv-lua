local 强化装备Module = ModuleBase:extend('强化装备02_01')
...
-- 260~268 为本客户端预留的潜能协议号；当前 customxb_handler.lua 尚未定义这些参数。
local 潜能协议 = {
    查询 = 260,
    放入装备 = 261,
    放入道具 = 262,
...
function 强化装备Module:onLoad()
    ...
    self:onPacketRecv('XBCENTER', function(header, params)
        ...
        self.缓存宝石文本 = 宝石文本
        if 悬停强化槽 then
            self:显示悬停提示(整体文本, 当前强化等级, 耐久文本, 种类文本, 套装文本, 宝石文本)
        end
        self:刷新强化槽显示()
    end)
    ...
    self:onPacketRecv('EQUIPINFO', function(header, params)
        ...
        if 图 > 0 and 文本 ~= '' then
            pcall(function() self:显示悬停提示(文本, params[3], params[5], params[6], params[7], params[8]) end)
        else
            self:显示装备格类型提示(格)
        end
    end)
...
function 强化装备Module:镶嵌格悬停(名称)
    ...
    if not 状态 or 状态.显示 ~= 1 or 状态.图号 <= 0 then return true end
    if 名称 == '宝石格' then
        self:显示悬停提示(self.缓存镶嵌文本 and self.缓存镶嵌文本.宝石格 or '', 状态.等级 or 0, '', '', '', '')
    else
        local 副 = self.缓存镶嵌副文本 and self.缓存镶嵌副文本[名称]
        self:显示悬停提示(self.缓存镶嵌文本 and self.缓存镶嵌文本[名称] or '', 状态.等级 or 0, 副 and 副.durab or '', 副 and 副.type or '', 副 and 副.set or '', 副 and 副.gem or '')
    end
    return true
end
...
function 强化装备Module:显示悬停提示(整体文本, 等级, 耐久文本, 种类文本, 套装文本, 宝石文本)
    if 悬停背包格 and 悬停背包格 > 0 then
        背包格种类[悬停背包格] = 种类文本
    end
    if 悬停银行格 and 悬停银行格 > 0 then
...
function 强化装备Module:银行格离开(i)
    if 悬停银行格 == i then 悬停银行格 = nil; self:隐藏悬停提示() end
    return true
end
...
function 强化装备Module:背包格离开(i)
    if 悬停背包格 == i then 悬停背包格 = nil; self:隐藏悬停提示() end
    return true
end
...
function 强化装备Module:装备格悬停(e)
    ...
    if 图 > 0 then self:sendPacket('CUSTOMXB 212 ' .. tostring(e)) else self:显示装备格类型提示(e) end
    return true
end

function 强化装备Module:装备格离开(e)
    if 悬停装备格 == e then 悬停装备格 = nil; self:隐藏悬停提示() end
    return true
end
...
function 强化装备Module:强化槽悬停()
    ...
    if 强化槽显示 == 1 and 强化槽图号 > 0 then
        self:显示悬停提示(self.缓存属性文本, 当前强化等级, self.缓存耐久文本, self.缓存种类文本, self.缓存套装文本, self.缓存宝石文本)
    else
        self:隐藏悬停提示()
    end
    return true
