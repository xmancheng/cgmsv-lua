...
function Module:onLoad()
  self:logInfo('load');
  --交换的训练家1
  self.petSwap1NPC = self:NPC_createNormal('天星隊皮拿', 231063, { x = 211, y = 294, mapType = 0, map = 20300, direction = 0 });
  self:NPC_regTalkedEvent(self.petSwap1NPC, function(npc, player)
    if (NLG.CanTalk(npc, player) == true) then
        local msg = "\\n\\n@c我好想要ooo，你如果有xxx\\n\\n確定要和我交換嗎？\\n\\n\\n\\n※注意！建議僅有1隻該寵物再進行交換";	
        NLG.ShowWindowTalked(player, self.petSwap1NPC, CONST.窗口_信息框, CONST.按钮_是否, 1, msg);
    end
    return
  end)
      ...
      elseif seqno == 1 and select == CONST.按钮_是 then
          if (PetIndex>0) then
              ...
              NLG.UpChar(player);
                 ...
                 NLG.SystemMessage(player,"皮拿:你沒有我要的寵物，我不要跟你交換。");
                 return;
          end
      else
                 return;
