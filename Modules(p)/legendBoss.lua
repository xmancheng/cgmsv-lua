...
function LegendBoss_LoopEvent(LegendBossNPC)
	if (os.date("%X",os.time())=="00:00:01") then
		NLG.SystemMessage(-1,"[系統]傳說寶可夢出現在(218,88)！");
		for k,v in pairs(LegendBoss) do
			local bossImage = Char.GetData(LegendBossNPC,CONST.对象_形象);
			if ( bossImage==v.startImage ) then
				Char.SetData(LegendBossNPC,CONST.对象_X, v.warpArea.X);
				Char.SetData(LegendBossNPC,CONST.对象_Y, v.warpArea.Y);
				Char.SetData(LegendBossNPC,CONST.对象_地图, v.warpArea.map);
				NLG.UpChar(LegendBossNPC);
			end
		end
	elseif (os.date("%X",os.time())=="23:59:59")  then
		for k,v in pairs(LegendBoss) do
			Char.SetData(tbl_LegendBossNPCIndex[k],CONST.对象_X, v.waitingArea.X);
			Char.SetData(tbl_LegendBossNPCIndex[k],CONST.对象_Y, v.waitingArea.Y);
			Char.SetData(tbl_LegendBossNPCIndex[k],CONST.对象_地图, v.waitingArea.map);
