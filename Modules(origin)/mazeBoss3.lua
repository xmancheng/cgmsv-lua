...
EnemySet[1] = {406188, 406187, 406187, 406186, 406186, 406185, 0, 0, 406184, 406184}    --0代表没有怪
BaseLevelSet[1] = {130, 120, 120, 120, 120, 120, 0, 0, 120, 120}
Pos[1] = {"賽克特",EnemySet[1],BaseLevelSet[1]}
------------------------------------------------
--背景设置
...
function Module:onLoad()
  ...
  local Lord3Npc = self:NPC_createNormal('時間裁決者·賽克特', 130059, { map = 7908, x = 51, y = 33, direction = 6, mapType = 0 })
  Char.SetData(Lord3Npc,CONST.对象_ENEMY_PetFlg+2,0)--可穿透体
  ...
  self:NPC_regWindowTalkedEvent(Lord3Npc, function(npc, player, _seqno, _select, _data)
	...
	if seqno == 1 then
		...
		elseif select == CONST.按钮_下一页 then  ----参加领主讨伐
			...
			--print(worldLayer)
			if(Char.ItemNum(player,46481)<=0) then
				NLG.SystemMessage(player,"[系統]看來你無法代表時之巫女，快滾吧。");
				return;
...
function Module:OnbattleStartEventCallback(battleIndex)
	...
	if (Char.GetData(player,CONST.对象_地图)~=7902) then
		return
	end
	--[[local ret = SQL.Run("select Name,WorldLord3 from lua_hook_worldboss where CdKey='"..cdk.."'");
	if(type(ret)=="table" and ret["0_1"]~=nil)then
		LordHP3=tonumber(ret["0_1"]);
	end]]
	if (cdk~=nil) then
		LordHP3 = tonumber(SQL.Run("select WorldLord3 from lua_hook_worldboss where CdKey='"..cdk.."'")["0_0"])
	else
		return
	end
	for i = 10, 19 do
	...
	for i = 10, 19 do
		...
		if Round>=0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==BossEnemyId  then
			...
			if (cdk~=nil) then
				...
				SQL.Run("update lua_hook_worldboss set WorldLord3= '"..HP.."' where CdKey='"..cdk.."'")
				NLG.UpChar(player);
			end
		end
	end
