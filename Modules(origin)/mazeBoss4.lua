...
function Module:onLoad()
  ...
  local Lord4Npc = self:NPC_createNormal('時間裁決者·賽克特', 130059, { map = 7903, x = 49, y = 50, direction = 6, mapType = 0 })
  ...
  self:NPC_regWindowTalkedEvent(Lord4Npc, function(npc, player, _seqno, _select, _data)
		...
		elseif select == CONST.按钮_下一页 then  ----参加领主讨伐
			...
			--else
				local msg = "\\n時間裁決者·賽克特：\\n\\n"
					...
					.."　王國的榮耀？勇者的傳說？他低笑抬手召喚\\n"
					.."　出一道虛影，昔日的戰爭場景在空氣中顯現——\\n"
					.."　城堡在烈焰中崩毀，無數生命在時間洪流中消逝\\n"
					.."　不……歷史是勝利者所書寫的謊言。\\n"
					.."　時間，只是他們用來維護權力的工具。\\n";
				NLG.ShowWindowTalked(player, npc, CONST.窗口_信息框, CONST.按钮_下一页, 11, msg);
			--end
		end
	...
	if seqno == 11 then  ----超級模式执行
		...
		elseif select == CONST.按钮_下一页 then
				local msg = "\\n時間裁決者·賽克特：\\n\\n"
					.."　他伸出手，裂隙內的龍魂碎片發出微弱的共鳴。\\n"
					.."　克羅諾斯的力量足以撕裂這虛偽的世界，若我能\\n"
					.."　掌握這股力量便能重塑歷史——讓羅連斯復甦\\n"
					.."　讓世界回到它應有的秩序！\\n"
					.."　(他的目光轉向你，眼神宛如劍鋒般銳利)\\n"
					.."　你選擇守護腐朽的時代？還是攜手推翻這一切？\\n";
				NLG.ShowWindowTalked(player, npc, CONST.窗口_信息框, CONST.按钮_是否, 12, msg);
	...
	elseif seqno == 12 then
		...
		elseif select == CONST.按钮_是 then
			...
			NLG.SystemMessage(player,"[系統]開啟龍魂之間，收集四散的龍魂碎片。");
		end
	end
  ...
  self:NPC_regTalkedEvent(Lord4Npc, function(npc, player)
	if (NLG.CanTalk(npc, player) == true) then
		local msg = "\\n時間裁決者·賽克特：\\n\\n"
				.."　這真是諷刺，艾露西亞(他的聲音低沉而嘲弄)\\n"
				.."　我們曾是同族，曾一同守護這條時間之流……\\n"
				.."　而如今你卻選擇順應命运，而我選擇改變命运。\\n"
				.."　他走下階梯步步逼近，裂隙的風圍繞著他\\n"
				.."　彷彿在回應他的呼喚！\\n";
	...
	for i = 10, 19 do
		...
		if Round>=0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==BossEnemyId  then
			...
			if (cdk~=nil) then
				...
				SQL.Run("update lua_hook_worldboss set WorldLord4= '"..HP.."' where CdKey='"..cdk.."'")
				NLG.UpChar(player);
			end
		end
	end
