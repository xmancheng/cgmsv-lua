...
EnemySet[1] = {406196, 0, 0, 0, 0, 0, 406194, 406193, 0, 0}    --0代表没有怪
BaseLevelSet[1] = {160, 0, 0, 0, 0, 0, 150, 150, 0, 0}
...
function Module:OnbattleStartEventCallback(battleIndex)
	...
	if (Char.GetData(player,CONST.对象_地图)~=7907 or Char.GetData(player,CONST.对象_地图)~=7906) then
		return
	end
	...
	if (cdk~=nil) then
		LordHP7 = tonumber(SQL.Run("select WorldLord7 from lua_hook_worldboss where CdKey='"..cdk.."'")["0_0"])
	else
		return
	end
	for i = 10, 19 do
