...
function initBravadoScriptNpc()
	if (BravadoScriptNps == nil) then
		...
		Char.SetData(BravadoScriptNps,%对象_原名%,"单人挑战道场");
	...
	if (BravadoScriptNpsB == nil) then
		...
		Char.SetData(BravadoScriptNpsB,%对象_原名%,"奖励发放员");
	...
	if (BravadoScriptNpsC == nil) then
		...
		Char.SetData(BravadoScriptNpsC,%对象_原名%,"明怪设置");
	...
	if (NLG.CanTalk(_me,_tome) == true) then
		if (Char.ItemNum(_tome,70211) > 0) then
			...
			if (win==2) then
				...
				NLG.SystemMessage(_tome, "[系统]已领取材料奖励！")
			else
				NLG.SystemMessage(_tome, "[系统]领取资格不符！！")
			end
		...
		if tPlayerIndex>=0 and Char.GetData(tPlayerIndex,%对象_类型%)==1 then
			if(Char.ItemNum(tPlayerIndex,70211) > 0) then
				...
				if (getamountless_2 >= 2)then
					NLG.SystemMessage(tPlayerIndex, "[系统]《单人挑战二连胜》成就达成！")
			...
			if(Char.ItemNum(tPlayerIndex,70212) > 0) then
				...
				NLG.SystemMessage(tPlayerIndex, "[系统]击败了"..Char.GetData(_NpcIndex, %对象_原名%).."获得1分,累积达到"..getamountless_3.."分")
				Char.GiveItem(tPlayerIndex,script_map_drop[3][drop],1);
				if (getamountless_3 >= 3)then
					NLG.SystemMessage(tPlayerIndex, "[系统]《单人挑战三连胜》成就达成！")
			...
			if(Char.ItemNum(tPlayerIndex,70213) > 0) then
				...
				NLG.SystemMessage(tPlayerIndex, "[系统]击败了"..Char.GetData(_NpcIndex, %对象_原名%).."获得1分,累积达到"..getamountless_4.."分")
				Char.GiveItem(tPlayerIndex,script_map_drop[4][drop],1);
				if (getamountless_4 >= 5)then
					NLG.SystemMessage(tPlayerIndex, "[系统]《单人挑战五连胜》成就达成！")
			...
			if(Char.ItemNum(tPlayerIndex,70214) > 0) then
				...
				NLG.SystemMessage(tPlayerIndex, "[系统]击败了"..Char.GetData(_NpcIndex, %对象_原名%).."获得1分,累积达到"..getamountless_5.."分")
				Char.GiveItem(tPlayerIndex,script_map_drop[5][drop],1);
				if (getamountless_5 >= 10)then
					NLG.SystemMessage(tPlayerIndex, "[系统]《单人挑战十连胜》成就达成！")
