...
function PetAttrib_create()
	if (PetAttribNPC == nil) then
		...
		Char.SetData(PetAttribNPC,%对象_地图%,1000);
		Char.SetData(PetAttribNPC,%对象_X%,224);
		Char.SetData(PetAttribNPC,%对象_Y%,83);
		Char.SetData(PetAttribNPC,%对象_方向%,4);
		Char.SetData(PetAttribNPC,%对象_名字%,"宠物洗档大师");
...
--		end
		if(Char.GetData(_PetIndex,%对象_名色%) ~= 0) then
			NLG.SystemMessage(_PlayerIndex,"[系统]无法对转生宠物进行洗档!");
			return;
		end
...
--		end
	...
	if a6 > 20 then
		NLG.SystemMessage(_PlayerIndex,"[系统]无法对白化宠物进行洗档!");
		return;
	end
	if (_SqeNo==1 and Char.ItemNum(_PlayerIndex,itemid_70052) == 0) then
		NLG.SystemMessage(_PlayerIndex,"[系统]请确认物品栏有洗檔道具");
		return;
	end
	if (_SqeNo==1 and Char.ItemNum(_PlayerIndex,itemid_70052) > 0) then
	...
	if (_SqeNo==12 and _select==8) then
		...
		NLG.SystemMessage(_PlayerIndex,"[系统]您的宠物 "..Char.GetData(_PetIndex,%对象_名字%).." 回复档次!");
	end
end
