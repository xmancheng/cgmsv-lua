...
------------------------------------------------------------------------------
local a1 = {%道具_攻击%,40,30,20};
local a2 = {%道具_防御%,40,30,20};
local a3 = {%道具_敏捷%,25,20,15};
local a4 = {%道具_精神%,10,7,4};
local a5 = {%道具_回复%,15,10,5};
local a6 = {%道具_HP%,150,100,50};
local a7 = {%道具_MP%,60,40,20};
local a8 = {%道具_魔攻%,40,30,20};
local a9 = {%道具_魔抗%,20,15,10};
...
local f1 = {%道具_必杀%,6,4,2};
local f2 = {%道具_反击%,6,4,2};
local f3 = {%道具_命中%,6,4,2};
local f4 = {%道具_闪躲%,6,4,2};
...
function AccessoriesCube_create() 
	if (exploitNPC == nil) then
		...
		Char.SetData(exploitNPC,%对象_名字%,"浅能开发专家");
...
function ExploitFunction(_NpcIndex,_PlayerIndex,_SqeNo,_select,_data)
	if ((_select == 0 or _select == "0") and (_data ~= "")) then
		...
		if (Special ~= 0 and Para1 == 7 and item_indexA ~= nill) then
			if(Para2 == 1) then
				Msg = "装饰品:  "..Item.GetData(item_indexA,%道具_名字%).."  \\n"
						.."浅  能:  一项  \\n";
