	...
	if (NLG.CanTalk(_NpcIndex,_PlayerIndex) == true) then
		...
		for slot=0,4 do
			...
			if (PetID == StrPetMutationEnable[i][2]) then
				NLG.SystemMessage(_PlayerIndex, "[系统]操作时宠物栏不能有變異宠物。")
				return;
			end
			end
		end
...
--		end
		if(Char.GetData(_PetIndex,%对象_名色%) ~= 0) then
			NLG.SystemMessage(_PlayerIndex,"[系统]无法对转生宠物进行异变!");
			return;
		end
...
--		end
		...
		a6 = a1 + a2+ a3+ a4+ a5;
		if a6 > 20 then
			NLG.SystemMessage(_PlayerIndex,"[系统]无法对白化宠物进行异变!");
			return;
		end
		local	PetInfoMsg = "宠 物 名:  "..Char.GetData(_PetIndex,%对象_名字%).."\\n"
					.."         【档  次  分  布】\\n"
					.."体力档数:  -"..a1.." 档      敏捷档数:  -"..a4.." 档 \\n"
					.."力量档数:  -"..a2.." 档      魔法档数:  -"..a5.." 档 \\n"
					.."防御档数:  -"..a3.." 档      掉 档 数:  -"..a6.." \\n"
	...
	if (_SqeNo==12 and _select==4) then
		if (Char.GetData(_PlayerIndex, %对象_金币%) < 5000) then
			NLG.SystemMessage(_PlayerIndex,"[系统]请确认金钱尚有每次的研究费用五千!");
			return;
		end
		...
		if ( M_Rate ~=1 and Char.ItemNum(_PlayerIndex,StrPetMutationEnable[EnemyNum][5]) < 20) then
			...
			NLG.SystemMessage(_PlayerIndex,"[系统]此次的变异研究失败!");
			return;
		end
		if ( M_Rate ~=1 and Char.ItemNum(_PlayerIndex,StrPetMutationEnable[EnemyNum][5]) >= 20) then
	...
	if (_SqeNo==5) then
		...
		if(selectitem == nil or selectitem > 4 or selectitem < 0) then
			NLG.SystemMessage(_PlayerIndex, "[系统]操作时宠物栏需有變異宠物。");
			return;
		end
		if(Char.ItemNum(_PlayerIndex,72000) < 100) then
			NLG.SystemMessage(_PlayerIndex, "[系统]強化須要異變核心至少100個。");
			return;
		end
		if(string.find(EnemyID,"+") ~= nill ) then
			NLG.SystemMessage(_PlayerIndex, "[系统]變異宠物已經強化過。");
			return;
		end
		if(EnemyID == StrPetMutationEnable[1][2] or EnemyID == StrPetMutationEnable[2][2] or EnemyID == StrPetMutationEnable[3][2] or 
			...
			Pet.UpPet(_PlayerIndex,_PetIndex);
			NLG.SystemMessage(_PlayerIndex, "[系统]變異宠物強化成功!");
		else
			NLG.SystemMessage(_PlayerIndex, "[系统]只有變異宠物可以強化。");
			return;
		end
	end
end
