...
function RecordFunction(_NpcIndex,_PlayerIndex,_SqeNo,_select,_data)
	if ((_select == 0 or _select == "0") and (_data ~= "")) then
		...
		if (Mappoint == 0 and record_MapType == 0 and MapEnable[record_MapId] == 0) then
			NLG.SystemMessage(_PlayerIndex,"此地图禁止纪录点。");
			return;
		end
		if (Mappoint == 0 and record_MapType == 0 and MapEnable[record_MapId] ~= 0) then
			...
			NLG.SystemMessage(_PlayerIndex,"坐标位置储存成功。");
		end
		...
		--end
		if (Gold < 1000) then
			NLG.SystemMessage(_PlayerIndex,"每次传送费用1000元。");
			return;
		end
		if (Mappoint ~= 0 and Gold >= 1000) then
			...
			NLG.SystemMessage(_PlayerIndex,"坐标纪录回复初始。");
		end
	end
end
