...
CardTable= {}
CardTable[1] = {%对象_体力%,5000,14,8,3,%对象_命中%,Plus1,"命中"};
CardTable[2] = {%对象_强度%,5000,14,8,1,%对象_反击%,Plus1,"反击"};
CardTable[3] = {%对象_速度%,5000,14,8,4,%对象_必杀%,Plus1,"必杀"};
CardTable[4] = {%对象_魔法%,5000,14,8,2,%对象_闪躲%,Plus1,"闪躲"};

CardTable[5] = {%对象_力量%,700,14,9,1,%对象_强度%,Plus,"强度"};
CardTable[6] = {%对象_力量%,700,14,9,2,%对象_体力%,Plus,"体力"};
CardTable[7] = {%对象_力量%,700,14,9,3,%对象_魔法%,Plus,"魔法"};
CardTable[8] = {%对象_力量%,700,14,9,4,%对象_速度%,Plus,"速度"};
...
function RegItemAttachEvent(player,item)
	...
	for i = 1,4 do
		...
		if (b >= CardTable[i][2] and Special == CardTable[i][3] and Para1 == CardTable[i][4] and Para2 == CardTable[i][5])then
			...
			NLG.SystemMessage(player,"☆卡片加成☆ 获得".. CardTable[i][8] .."修正属性".. CardTable[i][7] .."点额外加成");
		end
	end
	for j = 5,8 do
		...
		if (p >= CardTable[j][2] and Special == CardTable[j][3] and Para1 == CardTable[j][4] and Para2 == CardTable[j][5])then
			...
			Item.UpItem(item,Slot);
			NLG.SystemMessage(player,"☆卡片加成☆ 获得".. CardTable[j][8] .."能力属性".. CardTable[j][7]/100 .."点额外加成");
		end
	end
...
function RegItemDetachEvent(player,item)
	...
	for i = 1,4 do
		...
		if (Special == CardTable[i][3] and Para1 == CardTable[i][4] and Para2 == CardTable[i][5])then
			...
			NLG.SystemMessage(player,"☆卡片加成☆ 取消".. CardTable[i][8] .."修正属性".. Luck .."点额外加成");
		end
	end
	for j = 5,8 do
		...
		if (Special == CardTable[j][3] and Para1 == CardTable[j][4] and Para2 == CardTable[j][5])then
			...
			NLG.SystemMessage(player,"☆卡片加成☆ 取消".. CardTable[j][8] .."能力属性".. Luck/100 .."点额外加成");
		end
	end
	local Gold = Char.GetData(player, %对象_金币%);
	if(Item.GetData(item,%道具_类型%) == 22)then
