...
skillParams={
	...
	{skillId=3102,com1=CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,techId={310200,310202},side=1,unit=0},
	{skillId=3102,com1=CONST.BATTLE_COM.BATTLE_COM_M_BLOODATTACK,techId={310203,310205},side=1,unit=0},
	{skillId=3102,com1=CONST.BATTLE_COM.BATTLE_COM_DELAYATTACK,techId={310206,310208},side=1,unit=0},
	{skillId=3102,com1=CONST.BATTLE_COM.BATTLE_COM_AXEBOMBER,techId={310209,310211},side=1,unit=1},
	{skillId=3103,com1=CONST.BATTLE_COM.BATTLE_COM_ATTACKALL,techId={10627,10629},side=1,unit=1},
	{skillId=3103,com1=CONST.BATTLE_COM.BATTLE_COM_P_SPIRACLESHOT,techId={310303,310305},side=1,unit=0},
	{skillId=3103,com1=CONST.BATTLE_COM.BATTLE_COM_P_SPIRACLESHOT,techId={310306,310309},side=1,unit=0},
	{skillId=3103,com1=CONST.BATTLE_COM.BATTLE_COM_AXEBOMBER,techId={310309,310311},side=1,unit=2},
	-- 攻击
...
function Module:onLoad()
  ...
  self.petBankNPC = self:NPC_createNormal('寵物對戰倉庫', 14682, { x = 48, y = 63, mapType = 0, map = 80021, direction = 0 });
  ...
  self.ChessNPC = self:NPC_createNormal('寵物自動對戰', 14682, { x = 45, y = 40, mapType = 0, map = 80021, direction = 0 });
      ...
      if data==1 then
        ...
        for slot = 1, 3 do
          ...
          if type(petbagPet) == 'table' then
            ...
            if (a6>140) then
              NLG.SystemMessage(player, "[系統]寵物總檔次超過140無法入隊。");
              return
            end
  ...
  self.PvpRewardNPC = self:NPC_createNormal('對戰積分兌換', 400222, { map =80010, x = 92, y = 99, direction = 4, mapType = 0 })
  Char.SetData(self.PvpRewardNPC,CONST.对象_ENEMY_PetFlg+2,0)--可穿透体
  self:NPC_regWindowTalkedEvent(self.PvpRewardNPC, function(npc, player, _seqno, _select, _data)
    local cdk = Char.GetData(player,CONST.对象_CDK);
    local seqno = tonumber(_seqno)
    local select = tonumber(_select)
    local data = tonumber(_data)
    ...
    if seqno == 1 and data >= 0 then
        local pts = Char.GetExtData(player, '自走积分') or 0;
        if (pts>=item_list[data][3]) then
          Char.GiveItem(player, item_list[data][2], 1);
          Char.SetExtData(player, '自走积分', pts - item_list[data][3]);
          NLG.UpChar(player);
        else
          NLG.SystemMessage(player, "[系統]可兌換的積分不足。");
          return
        end
    end
  end)
  self:NPC_regTalkedEvent(self.PvpRewardNPC, function(npc, player)
    if (NLG.CanTalk(npc, player) == true) then
      local msg = "5\\n\\n@c★玩家對戰★"
                .."\\n交出所須的積分點數獲得指定道具\\n"
                .."\\n　　————————————————————\\n";
      for i=1,#item_list do
        msg = msg .."　"..item_list[i][1].."　　　積分"..item_list[i][3].."點\\n";
      end
      NLG.ShowWindowTalked(player, npc, CONST.窗口_选择框, CONST.按钮_关闭, 1, msg);
    end
    return
...
function Module:onLoginEvent(charIndex)
  ...
  if (Char.GetData(charIndex, CONST.对象_地图)==80021) then
    Char.Warp(charIndex,0,80010,103,103);
  end
end
...
function Module:onBattleOver(battleIndex)
	for i=0,9 do
		...
		if (player>-1 and Char.GetData(player, CONST.对象_地图)==80021) then
