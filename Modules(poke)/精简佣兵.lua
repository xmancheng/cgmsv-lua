...
function 精简佣兵Module:onLoad()
	...
	self:regCallback('PartyEvent', Func.bind(self.组队触发, self))--组队触发
...
--{1,'恶魔猎手','弓箭手',108314,44,40,215,'不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|15|200|999',100},
--{2,'剑圣','剑士',108299,14,10,201,'不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|200|999',100},
--{3,'法国榔头','战斧斗士',108246,24,20,206,'不改|不改|不改|不改|不改|不改|不改|100|不改|不改|不改|不改|200|999',100},
--{4,'Archer','骑士',170006,34,30,211,'不改|不改|不改|不改|不改|不改|不改|100|不改|70|不改|不改|200|999',100},
--{5,'今野明美','格斗士',102833,144,140,250,'不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|15|200|999',100},
--{6,'马猴烧酒','魔法师',102847,74,70,220,'不改|不改|不改|不改|不改|1000|350|不改|不改|不改|不改|不改|200|999',100},
--{7,'不知火舞','传教士',102918,64,60,220,'500|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|800|999',200},
--{8,'奶大先说话','巫师',120073,134,130,220,'500|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|不改|800|999',200},
...
local 宠物名色 = 99--如果你的服，有给玩家宠物用到namecolor，请错开编号。此编号用于判断是否为ai佣兵的宠物，然后自动战斗。
local boss战是否允许暗杀 = true--true允许，false不允许
local 开通币种 = 0--0魔币，1自定义币
local 自定义币名 = 'CN币'
...
local 在线玩家表 = NLG.GetPlayer() or false--获取在线玩家表
if 在线玩家表 ~= -1 then--reload清理
	for k,v in pairs(在线玩家表) do
		if Char.IsDummy(v) then
			for i = 1,#ai_list do
				if Char.GetData(v,CONST.对象_名字) == ai_list[i][2] then
					Char.LeaveParty(v)
					Char.DelDummy(v)
				end
			end
...
function 精简佣兵Module:面向npc触发(npc,player)--面向ai_npc触发
	...
	if 现在页[player]-1 == 0 then--首页
		NLG.ShowWindowTalked(player,npc,CONST.窗口_选择框,CONST.按钮_关闭+CONST.按钮_下一页,player+3330,窗口内容)
...
function 精简佣兵Module:玩家说话(player,msg,color,range,size)--玩家说话
	...
	if msg == '/aiback' or msg == '、aiback' then
		...
		for i = 1,#ai_list do
			if Char.PartyNum(玩家持有ai表[player][i]) == -1 then
				Char.SimpleLogout(玩家持有ai表[player][i])
	...
	if msg == '/killai' or msg == '、killai' then--干掉ai的命令
		...
		NLG.SystemMessage(player, '已开除所有雇佣兵。' )
		return 0
	end
	return 1--pass talk
end
...
function 精简佣兵Module:战斗结束触发(battleindex)--战后触发
	...
	for i = 0,9 do--获取playerindex
		local player = Battle.GetPlayer(battleindex,i) or -1
		if Char.IsPlayer(player) then--玩家判定
			self:玩家说话(player,'/aiback')
		end
	end
	return 0
...
function 精简佣兵Module:回合开始前触发(battleindex)
	...
	for i = 0, 9 do
		...
		if ai_index >= 0 then
			...
			if Char.IsDummy(ai_index) then--ai判断
				...
				player[i] = ai_index
				--print('ai名字 = '..Char.GetData(ai_index,CONST.对象_名字))
				Char.SetData(ai_index,CONST.对象_魔,Char.GetData(ai_index,CONST.对象_最大魔))--确保ai一直有魔够无限放技能
				local 等待指令 = Battle.IsWaitingCommand(ai_index)
				if 等待指令 == 1 then--等待指令
					--print('等待指令')
					...
					if 职业类 == 40 then--弓
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RANDOMSHOT,math.random(10,19),9509) end,--乱射
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_DELAYATTACK,math.random(10,19),25809) end,--一击必中
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_DODGE,math.random(10,19),909) end,--阳炎
						}
					...
					if 职业类 == 10 then--剑
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_FIRSTATTACK,math.random(10,19),26209) end,--迅速
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_BLASTWAVE,math.random(10,19),200509) end,--剑气--请确认你的tech也是这个编号
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),309) end,--乾坤
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),109) end,--诸刃
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),9) end,--连击破碎
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),8) end,--连击乱舞
						}
						local 随机施放 = math.random(1,#技能表)
						pcall(技能表[随机施放])
					end
					if 职业类 == 20 then--斧
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_DELAYATTACK,math.random(10,19),25709) end,--戒骄戒躁
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),309) end,--乾坤
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),109) end,--诸刃
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),9) end,--连击破碎
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),8) end,--连击乱舞
					...
					if 职业类 == 30 then--骑
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_BILLIARD,math.random(15,19),26009) end,--一石二鸟
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),309) end,--乾坤
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),109) end,--诸刃
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),9) end,--连击破碎
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),8) end,--连击乱舞
					...
					if 职业类 == 140 then--格
						local 技能表 = {
							function() Battle.ActionSelect(ai_index, CONST.BATTLE_COM.BATTLE_COM_P_SPIRACLESHOT,math.random(10,19),409) end,--气功弹
							function() Battle.ActionSelect(ai_index, CONST.BATTLE_COM.BATTLE_COM_P_PANIC,math.random(10,19),9409) end,--混乱攻击
							function() Battle.ActionSelect(ai_index, CONST.BATTLE_COM.BATTLE_COM_P_GUARDBREAK,math.random(10,19),509) end,--崩击
						}
					...
					if 职业类 == 70 then--魔
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,41,2709) end,--超陨
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,41,2809) end,--超冰
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,41,2909) end,--超火
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,41,3009) end,--超风
						}
						local 随机施放 = math.random(1,#技能表)
						pcall(技能表[随机施放])
					end
					if 职业类 == 60 then--传
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_HEAL,40,6309) end,--超强补血
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_HEAL,math.random(20,29),6209) end,--强力补血
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_HEAL,math.random(0,9),6109) end,--补血
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_REVIVE,math.random(0,9),6809) end,--气绝
						}
					...
					if 职业类 == 130 then--巫
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_LP_RECOVERY,40,6609) end,--超强恢复
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSRECOVER,40,6709) end,--洁净
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_REVIVE,math.random(0,9),6809) end,--气绝
						}
						local 随机施放 = math.random(1,#技能表)
						pcall(技能表[随机施放])
					...
					if 职业类 == 80 then--咒
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4409) end,--超强毒
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4509) end,--超强睡
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4609) end,--超强石
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4709) end,--超强醉
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4809) end,--超强乱
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_STATUSCHANGE,41,4909) end,--超强忘
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_REVERSE_TYPE,math.random(0,19),5409) end,--属性反转
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_REFLECTION_PHYSICS,math.random(0,9),5509) end,--攻反
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_INEFFECTIVE_PHYSICS,math.random(0,9),5909) end,--攻无
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_ABSORB_PHYSICS,math.random(0,9),5709) end,--攻吸
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_REFLECTION_MAGIC,math.random(0,9),5609) end,--魔反
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_INEFFECTIVE_MAGIC,math.random(0,9),6009) end,--魔无
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_ABSORB_MAGIC,math.random(0,9),5809) end,--魔吸
						}
					...
					if 职业类 == 150 then--忍
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_ASSASSIN,math.random(10,19),9609) end,--暗杀
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_DODGE,math.random(10,19),909) end,--阳炎
						...
						if Battle.GetType(battleindex) == CONST.战斗_BOSS战 and not boss战是否允许暗杀 then--boss战，不允许暗杀
					...
					if Char.GetData(ai_index,CONST.对象_职业) == 481 then--GM测试
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_ASSASSIN,math.random(10,19),9609) end,--暗杀
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_M_DEATH,math.random(0,19),8602) end,--单体即死--请确认你的tech也是这个编号
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_M_EARTHQUAKE,math.random(40,41),9708) end,--大地之怒--请确认你的tech也是这个编号
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_P_DANCE,-1,9802) end,--变大
						}
						if Battle.GetType(battleindex) == CONST.战斗_BOSS战 and not boss战是否允许暗杀 then--boss战，不允许暗杀
							local 随机施放 = math.random(2,#技能表)
							pcall(技能表[随机施放])
					...
					if petindex < 0 or 出战宠物slot == -1 then--如果没带宠物，ai人第2动
						local 技能表 = {
							function() Battle.ActionSelect(ai_index,CONST.BATTLE_COM.BATTLE_COM_ATTACK,10,-1) end,--攻击
			...
			if Char.IsPet(ai_index) then--判定为宠物
				...
				local 名色 = Char.GetData(petindex,CONST.对象_名色)
				--print('名色 = '..名色)
				local 等待指令 = Battle.IsWaitingCommand(petindex)
				if 等待指令 == 1 and 名色 == 宠物名色 then
					Char.SetData(petindex,CONST.对象_魔,Char.GetData(petindex,CONST.对象_最大魔))--确保ai一直有魔够无限放技能
					...
					local 宠物技能表 = {--宠物施放的技能，需要宠物真的有学这个技能，否则无法施放，最多10个，人不用学，也无限制
						...
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_RENZOKU,math.random(10,19),38) end,--连击
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),138) end,--诸刃
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_PARAMETER,math.random(10,19),338) end,--乾坤
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,math.random(30,39),2339) end,--强力陨石魔法
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,math.random(30,39),2439) end,--强力冰冻魔法
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,math.random(30,39),2539) end,--强力火焰魔法
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_P_MAGIC,math.random(30,39),2639) end,--强力风刃魔法
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_M_BLOODATTACK,math.random(10,19),8138) end,--吸血攻击
						function() Battle.ActionSelect(petindex,CONST.BATTLE_COM.BATTLE_COM_M_STATUSATTACK,math.random(10,19),7738) end,--石化攻击
					}
	...
	--[[if Battle.GetType(battleindex) == CONST.战斗_PVP then--PK则ai跑路
...
function 创建ai(player,ai序,ai名字,ai职业显示,ai造型,ai职业,ai职业类,ai装备,ai装备篡改参数)
	...
	print('出战宠物slot：'..Char.GetData(ai_index,CONST.对象_战宠))
	...
	Char.SetData(petindex,CONST.对象_名色,宠物名色)
...
function 精简佣兵Module:组队触发(ai,leader,class)--组队触发
	if Char.IsDummy(ai) then
		if class == 0 then--加入队伍触发
			NLG.UpChar(ai)--刷新hmp
		end
	end
	return 1
...
function 治疗ai(player)--治疗ai
	for i = 1,4 do
		...
		if Char.IsDummy(ai) then--ai判定
			...
			Char.SetData(ai, CONST.CHAR_魅力, 100)
			local 装备index = false
