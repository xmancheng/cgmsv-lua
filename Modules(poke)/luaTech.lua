    ...
    if (skilltech==2009) then
      ...
      if (targetEnemy_Com1==1 or targetEnemy_Com1==43) then		--GUARD=0x1.SPECIALGARD=0x2B
        ...
        if ( cri_rate >= math.random(1,100) ) then
          ...
          if ( Char.GetData(targetEnemy,CONST.对象_血)<=0 ) then
            ...
            if (targetEnemy_Com1==1) then
              local dmg = dmg*0.8;
              DMGFunc(e,6,dmg);		--必殺防禦
            elseif (targetEnemy_Com1==43) then
              local dmg = dmg*0.5;
              DMGFunc(e,1026,dmg);		--必殺聖盾
            end
          end
        else
          if ( Char.GetData(targetEnemy,CONST.对象_血)<=0 ) then
            ...
            if (targetEnemy_Com1==1) then
              local dmg = dmg*0.8;
              DMGFunc(e,5,dmg);			--普通防禦
            elseif (targetEnemy_Com1==43) then
              local dmg = dmg*0.5;
              DMGFunc(e,1025,dmg);		--普通聖盾
            end
          end
        end
        --命中時給目標設置數據(特殊效果)
      ...
      elseif (targetEnemy_Com1==19 or targetEnemy_Com1==41) then		--EDGE=0x13.P_DODGE=0x29
        if (targetEnemy_Com1==41) then 
          DMGFunc(e,256,0);		--陽炎閃避
        else 
          DMGFunc(e,8,0);		--普通閃避
        end
      elseif (skilltype==1 and (targetEnemy_Com1==51 or targetEnemy_Com1==52 or targetEnemy_Com1==53)) then		--攻反吸無0x33~0x35
      ...
      elseif (skilltype==3 and (targetEnemy_Com1==54 or targetEnemy_Com1==55 or targetEnemy_Com1==56)) then		--魔吸反無0x36~0x38
