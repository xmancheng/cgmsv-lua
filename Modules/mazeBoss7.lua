...
--     十(9)	八(7)	六(5)	七(6)	九(8)
------------对战NPC设置------------
EnemySet[1] = {0, 406227, 406228, 406229, 406230, 0, 406227, 406228, 406229, 406230}    --0代表没有怪
EnemySet[2] = {406231, 406231, 406231, 406231, 406231, 406232, 406232, 406232, 406232, 406232}
EnemySet[3] = {0, 0, 0, 0, 0, 406233, 0, 0, 0, 0}
BaseLevelSet[1] = {0, 160, 160, 160, 160, 0, 160, 160, 160, 160}
BaseLevelSet[2] = {170, 170, 170, 170, 170, 170, 170, 170, 170, 170}
BaseLevelSet[3] = {0, 0, 0, 0, 0, 170, 0, 0, 0, 0}
Pos[1] = {"水脈魷魚",EnemySet[1],BaseLevelSet[1]}
Pos[2] = {"魷魚霸王",EnemySet[2],BaseLevelSet[2]}
Pos[3] = {"魷魚霸王",EnemySet[3],BaseLevelSet[3]}

------------------------------------------------
--背景设置
local Switch = 1;                          --组队人数限制开关1开0关
local Rank = 0;                             --初始化难度分类
local BossMap= {60015,21,19} -- 战斗场景Floor,X,Y(弱、普、超同场景)
...
local BossRoom = {
      { key=1, keyItem=70195, keyItem_count=1, bossRank=1, limit=-1, posNum_L=1, posNum_R=2,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70269, getItem_count = 1},
          lordName="水脈魷魚",
       },    -- 虚弱(1)
      { key=3, keyItem=70195, keyItem_count=1, bossRank=2, limit=3, posNum_L=2, posNum_R=3,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70269, getItem_count = 6},
          lordName="魷魚霸王",
       },    -- 普通(2)
...
function Module:onLoad()
  ...
  local Lord7Npc = self:NPC_createNormal('區域領主討伐', 11394, { map = 60014, x = 50, y = 50, direction = 6, mapType = 0 })
  self:regCallback('LoopEvent', Func.bind(self.AutoLord_LoopEvent,self))
  self:NPC_regWindowTalkedEvent(Lord7Npc, function(npc, player, _seqno, _select, _data)
	...
	if seqno == 1 then
		if data == 1 then  ----参加领主讨伐
			...
			if(Char.ItemNum(player,BossKey[1])>0 or Char.ItemNum(player,BossKey[2])>0 or Char.ItemNum(player,BossKey[3])>0) then
				...
				return;
			elseif (Char.ItemNum(player,70194)<=0 or Char.ItemNum(player,16443)<=0) then
				NLG.SystemMessage(player,"[系統]討伐需要1個捉迷藏達人券來打破結界。");
				NLG.SystemMessage(player,"[系統]此處刻著色慾印記，似乎須要色慾的罪書來共鳴。");
				return;
			else
  ...
  self:NPC_regTalkedEvent(Lord7Npc, function(npc, player)
    if (NLG.CanTalk(npc, player) == true) then
               local msg = "4\\n\\n@c★超級魔物的領地位在最深處★\\n"
                                             .."\\n　　════════════════════\\n"
                                             .."[　參加領主討伐　]\\n" 
  ...
  local Leave7Npc = self:NPC_createNormal('逃離沙漏', 235179, { map = 60015, x = 23, y = 22, direction = 6, mapType = 0 })
  self:NPC_regWindowTalkedEvent(Leave7Npc, function(npc, player, _seqno, _select, _data)
  end)
  self:NPC_regTalkedEvent(Leave7Npc, function(npc, player)
	if (NLG.CanTalk(npc, player) == true) then
  ...
  local Num = NLG.Rand(1,4);
  local fx1= tonumber(HideSeekLocation[Num][1]);
  local fx2= tonumber(HideSeekLocation[Num][3]);
  local fy1= tonumber(HideSeekLocation[Num][2]);
  local fy2= tonumber(HideSeekLocation[Num][4]);
  local MX = NLG.Rand(fx1, fx2);
  local MY = NLG.Rand(fy1, fy2);
  if tbl_HideSeekNpc==0 then
  local HideSeekNpc = self:NPC_createNormal('捉迷藏的罪匠鍛造師', 108315, { map = 60014, x = MX, y = MY, direction = 5, mapType = 0 })
  tbl_HideSeekNpc=1;
  self:NPC_regWindowTalkedEvent(HideSeekNpc, function(npc, player, _seqno, _select, _data)
          local cdk = Char.GetData(player,CONST.对象_CDK);
          local seqno = tonumber(_seqno);
          local select = tonumber(_select);
          local data = tonumber(_data);
          if seqno == 1 then
                if select == CONST.BUTTON_是 then
                      local Num2 = NLG.Rand(1,4);
                      local fx1= tonumber(HideSeekLocation[Num2][1]);
                      local fx2= tonumber(HideSeekLocation[Num2][3]);
                      local fy1= tonumber(HideSeekLocation[Num2][2]);
                      local fy2= tonumber(HideSeekLocation[Num2][4]);
                      local MX = NLG.Rand(fx1, fx2);
                      local MY = NLG.Rand(fy1, fy2);
                      Char.SetData(HideSeekNpc,CONST.CHAR_X,MX);
                      Char.SetData(HideSeekNpc,CONST.CHAR_Y,MY);
                      NLG.UpChar(HideSeekNpc);
                      NLG.UpChar(player);
                      if (Char.NowEvent(player, 306) == 0 and Char.NowEvent(player, 307) == 0) then
                          Char.NowEvent(player, 307,1);
                          NLG.SystemMessage(player,"哎呀！被你抓到了，咱換個地方躲起來。");
                      elseif (Char.NowEvent(player, 306) == 0 and Char.NowEvent(player, 307) == 1) then
                          Char.NowEvent(player, 306,1);
                          NLG.SystemMessage(player,"竟然又被你找到，不要煩人家好玩的遊戲嘛！");
                      elseif (Char.NowEvent(player, 306) == 1 and Char.NowEvent(player, 307) == 1) then
                          Char.NowEvent(player, 306,0);
                          Char.NowEvent(player, 307,0);
                          Char.GiveItem(player, 70194, 1);
                          NLG.SystemMessage(player,"給你鑰匙去挑戰領主，別再來偷偷尋找我！");
                      end
                end
          end
  end)
  self:NPC_regTalkedEvent(HideSeekNpc, function(npc, player)
        if (NLG.CanTalk(npc, player) == true) then
              local msg = "\\n\\n\\n@c★超級魔物的領地位在最深處★\\n"
                                                       .."\\n想要挑戰領主就跟咱玩捉迷藏\\n"
                                                       .."\\n找到躲起來的咱3次就給你鑰匙\\n";
...
function Module:OnbattleStartEventCallback(battleIndex)
	local Sum=0;
	local ret = SQL.Run("select Name,WorldLord7 from lua_hook_worldboss order by WorldLord7 asc limit 3");
	...
	for i = 10, 19 do
		...
		local HP = LordHP7;
		if enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
...
function Module:OnBeforeBattleTurnCommand(battleIndex)
	...
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		local HP = LordHP7;
		if Round==0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
			Char.SetData(enemy, CONST.CHAR_防御力, 420);
		elseif Round>0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
			Char.SetData(enemy, CONST.CHAR_防御力, 420);
			if Round>=5 then
				Char.SetData(enemy, CONST.CHAR_攻击力, 6000);
				Char.SetData(enemy, CONST.CHAR_精神, 6000);
				Char.SetData(enemy, CONST.CHAR_命中, 50);
				Char.SetData(enemy, CONST.CHAR_闪躲, 40);
				Char.SetData(enemy, CONST.CHAR_反击, 80);
			end
			if NLG.Rand(1,10)>=1  then
				local BuffData = {CONST.CHAR_BattleModDrunk};
				Char.SetData(enemy, BuffData[1], 1);
			end
			NLG.UpChar(enemy);
...
function Module:OnEnemyCommandCallBack(battleIndex, side, slot, action)
      ...
      for i = 10, 19 do
            local enemy = Battle.GetPlayer(battleIndex, i);
            if Round>=2 and Round<=15 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406232  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_BOMB, 40, 8200);
            elseif Round>=0 and Round<=9 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_SUMMON, -1, 9304);
            elseif Round>=10 and Round<=14 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_DEATH, 40, 8609);
            elseif Round>=15 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406233  then
