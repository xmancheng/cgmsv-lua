      ...
      --self:logDebug('OnDamageCalculateCallBack', charIndex, defCharIndex, oriDamage, damage, battleIndex, com1, com2, com3, defCom1, defCom2, defCom3, flg)
         ...
         if flg ~= CONST.DamageFlags.Miss and flg ~= CONST.DamageFlags.Dodge and Char.GetData(charIndex, CONST.CHAR_类型) == CONST.对象类型_人  then
               ...
               if ( RaceType == CONST.种族_人型 ) then 
                 ...
                 --NLG.Say(-1,-1,"人型系人物使目标接下来受到的伤害提高，此效果在战斗中每次打击提升1.5%，每回合重置次数",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_飞行 ) then 
                 ...
                 print(damage)
                 --NLG.Say(-1,-1,"飞行系人物每次造成伤害时，有30%的几率对当前生命值少于50%的目标伤害提高20%",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_昆虫 ) then 
                 ...
                 print(damage)
                 --NLG.Say(-1,-1,"昆虫系人物每次造成伤害时，有30%的几率对目标造成额外的等同于目标当前生命值一半的中毒伤害",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_野兽 ) then 
                 ...
                 --NLG.Say(-1,-1,"野兽系人物使目标受到最大生命值10%的出血伤害",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_特殊 ) then 
                 ...
                 print(damage)
                 --NLG.Say(-1,-1,"特殊系人物当最大血量低于50%，对目标造成损失血量%的复仇伤害",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_金属 ) then 
                 ...
                 --NLG.Say(-1,-1,"金属系人物造成伤害时，额外对目标造成相当于最大血量4.5%的伤害",4,3);
                 return damage;
               end
         else
         end
