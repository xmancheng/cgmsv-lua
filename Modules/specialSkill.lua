      ...
      --self:logDebug('OnDamageCalculateCallBack', charIndex, defCharIndex, oriDamage, damage, battleIndex, com1, com2, com3, defCom1, defCom2, defCom3, flg)
         ...
         if flg ~= CONST.DamageFlags.Miss and flg ~= CONST.DamageFlags.Dodge and flg ~= CONST.DamageFlags.Magic and Char.GetData(charIndex, CONST.CHAR_类型) == CONST.对象类型_人  then
               ...
               if ( RaceType == CONST.种族_人型) then 
                 ...
                 --NLG.Say(-1,-1,"人型系人物每次伤害随机提升1.5%~15%，有30%的几率自身攻反状态1回合，10%的几率使目标属反2回合，对已属反目标伤害提高30%",4,3);
                 return damage;
               end
               if ( RaceType == CONST.种族_龙) then 
                 local battleturn = Battle.GetTurn(battleIndex);
                 local defHp = Char.GetData(charIndex,CONST.CHAR_血);
                 local defMpE = Char.GetData(defCharIndex,CONST.CHAR_魔);
                 local defMpEM = Char.GetData(defCharIndex,CONST.CHAR_最大魔);
                 ...
                 if Char.GetData(defCharIndex, CONST.CHAR_BattleModDrunk)>=1  then
                        ...
                        if Char.GetData(leader,%对象_队聊开关%) == 1  then
                               NLG.Say(charIndex,charIndex,"【威壓】！！",4,3);
                        end
                 else
                        damage = damage * wy;
                        Char.SetData(defCharIndex, CONST.CHAR_BattleModDrunk, 1);
                        if Char.GetData(leader,%对象_队聊开关%) == 1  then
                               NLG.Say(charIndex,charIndex,"【龍息】！！",4,3);
                        end
                 end
                 --print(damage)
                 --NLG.Say(-1,-1,"龙系人物每次造成伤害时，对非酒醉的目标酒醉1回合，魔法值少于50%的目标伤害提高20%，对已酒醉目标额外受到我方血量值50%的恐惧伤害",4,3);
                 return damage;
               end
