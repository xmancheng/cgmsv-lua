...
--     十(9)	八(7)	六(5)	七(6)	九(8)
------------对战NPC设置------------
EnemySet[1] = {400075, 400075, 400075, 400075, 400075, 400074, 400074, 400074, 400074, 400074}    --0代表没有怪
EnemySet[2] = {0, 400075, 400075, 0, 0, 406191, 0, 0, 400074, 400074}
EnemySet[3] = {0, 400075, 400075, 0, 0, 406191, 0, 0, 400074, 400074}
BaseLevelSet[1] = {110, 110, 110, 110, 110, 110, 110, 110, 110, 110}
BaseLevelSet[2] = {0, 115, 115, 0, 0, 120, 0, 0, 115, 115}
BaseLevelSet[3] = {0, 115, 115, 0, 0, 120, 0, 0, 115, 115}
Pos[1] = {"遺跡黏液",EnemySet[1],BaseLevelSet[1]}
Pos[2] = {"液態史伊",EnemySet[2],BaseLevelSet[2]}
Pos[3] = {"液態史伊",EnemySet[3],BaseLevelSet[3]}
------------------------------------------------
--背景设置
local Switch = 1;                          --组队人数限制开关1开0关
local Rank = 0;                             --初始化难度分类
local BossMap= {60005,18,17} -- 战斗场景Floor,X,Y(弱、普、超同场景)
...
local BossRoom = {
      { key=1, keyItem=70195, keyItem_count=1, bossRank=1, limit=-1, posNum_L=1, posNum_R=2,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70259, getItem_count = 1},
          lordName="遺跡黏液",
       },    -- 虚弱(1)
      { key=3, keyItem=70195, keyItem_count=1, bossRank=2, limit=3, posNum_L=2, posNum_R=3,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70259, getItem_count = 6},
          lordName="液態史伊",
       },    -- 普通(2)
...
function Module:onLoad()
  ...
  local Lord2Npc = self:NPC_createNormal('區域領主討伐', 11394, { map = 60004, x = 57, y = 54, direction = 6, mapType = 0 })
  ...
  local Leave2Npc = self:NPC_createNormal('逃離沙漏', 235179, { map = 60005, x = 17, y = 20, direction = 6, mapType = 0 })
  self:NPC_regWindowTalkedEvent(Leave2Npc, function(npc, player, _seqno, _select, _data)
  end)
  self:NPC_regTalkedEvent(Leave2Npc, function(npc, player)
	if (NLG.CanTalk(npc, player) == true) then
...
function Module:OnbattleStartEventCallback(battleIndex)
	local ret = SQL.Run("select Name,WorldLord2 from lua_hook_worldboss order by WorldLord2 asc limit 3");
	...
	for i = 10, 19 do
		...
		local HP = LordHP2;
		if enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406191  then
...
function Module:OnBeforeBattleTurnCommand(battleIndex)
	...
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		local HP = LordHP2;
		if Round==0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406191  then
...
function Module:OnEnemyCommandCallBack(battleIndex, side, slot, action)
      ...
      for i = 10, 19 do
            local enemy = Battle.GetPlayer(battleIndex, i);
            if Round>=5 and Round<=9 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406191  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_DEATH, 40, 8609);
            elseif Round>=10 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406191  then
