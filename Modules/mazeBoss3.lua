---模块类
local Module = ModuleBase:createModule('mazeBoss3')
...
--     十(9)	八(7)	六(5)	七(6)	九(8)
------------对战NPC设置------------
EnemySet[1] = {400077, 400077, 400077, 400077, 400077, 400076, 400076, 400076, 400076, 400076}    --0代表没有怪
EnemySet[2] = {0, 400077, 400077, 0, 0, 406192, 0, 0, 400076, 400076}
EnemySet[3] = {400077, 400077, 400077, 400077, 400077, 400076, 0, 0, 400076, 400076}
EnemySet[4] = {0, 0, 0, 0, 0, 0, 406192, 406192, 0, 0}
BaseLevelSet[1] = {120, 120, 120, 120, 120, 120, 120, 120, 120, 120}
BaseLevelSet[2] = {0, 125, 125, 0, 0, 130, 0, 0, 125, 125}
BaseLevelSet[3] = {120, 120, 120, 120, 120, 120, 0, 0, 120, 120}
BaseLevelSet[4] = {0, 0, 0, 0, 0, 0, 130, 130, 0, 0}
Pos[1] = {"月影熊狐",EnemySet[1],BaseLevelSet[1]}
Pos[2] = {"夜地獄星",EnemySet[2],BaseLevelSet[2]}
Pos[3] = {"月影熊狐",EnemySet[3],BaseLevelSet[3]}
Pos[4] = {"夜地獄星",EnemySet[4],BaseLevelSet[4]}
------------------------------------------------
--背景设置
local Switch = 1;                          --组队人数限制开关1开0关
local Rank = 0;                             --初始化难度分类
local BossMap= {60007,23,21} -- 战斗场景Floor,X,Y(弱、普、超同场景)
...
local BossRoom = {
      { key=1, keyItem=70195, keyItem_count=1, bossRank=1, limit=-1, posNum_L=1, posNum_R=2,
          win={warpWMap=60006, warpWX=91, warpWY=24, getItem = 70261, getItem_count = 1},
          lordName="月影熊狐",
       },    -- 虚弱(1)
      { key=3, keyItem=70195, keyItem_count=1, bossRank=2, limit=3, posNum_L=2, posNum_R=3,
          win={warpWMap=60006, warpWX=91, warpWY=24, getItem = 70261, getItem_count = 6},
          lordName="夜地獄星",
       },    -- 普通(2)
      { key=5, keyItem=70195, keyItem_count=1, bossRank=3, limit=5, posNum_L=3, posNum_R=5,
          win={warpWMap=60006, warpWX=91, warpWY=24, getItem = 70206, getItem_count = 25},
          lordName="夜地獄星",
       },    -- 超级(3)
}
...
function Module:onLoad()
  ...
  local Lord3Npc = self:NPC_createNormal('區域領主討伐', 11394, { map = 60006, x = 67, y = 38, direction = 6, mapType = 0 })
  self:regCallback('LoopEvent', Func.bind(self.AutoLord_LoopEvent,self))
  self:NPC_regWindowTalkedEvent(Lord3Npc, function(npc, player, _seqno, _select, _data)
	...
	if seqno == 1 then
		if data == 1 then  ----参加领主讨伐
			local retEnd = SQL.Run("select Name,LordEnd3 from lua_hook_worldboss order by LordEnd3 desc ");
			...
			if(Char.ItemNum(player,BossKey[1])>0 or Char.ItemNum(player,BossKey[2])>0 or Char.ItemNum(player,BossKey[3])>0) then
				...
				return;
			elseif (Char.ItemNum(player,16446)<=0) then
				NLG.SystemMessage(player,"[系統]此處刻著憤怒印記，似乎須要憤怒的罪書來共鳴。");
				return;
			else
	...
	if seqno == 11 then  ----超級模式执行
		...
		if key == data+4 then
			if ret and #WorldDate > 0 then
				if WorldDate[3][1]==os.date("%w",os.time()) then
			...
			for k,v in pairs(BossRoom) do
				if ( key==v.key ) then
					...
					Char.SetLoopEvent('./lua/Modules/mazeBoss3.lua','AutoLord_LoopEvent',player,1000);
					--WorldDate = {}
					WorldDate[3] = {
					os.date("%w",os.time()),
					}
  ...
  self:NPC_regTalkedEvent(Lord3Npc, function(npc, player)
    if (NLG.CanTalk(npc, player) == true) then
               local msg = "4\\n\\n@c★超級魔物的領地位在最深處★\\n"
                                             .."\\n　　════════════════════\\n"
                                             .."[　參加領主討伐　]\\n" 
  ...
  local Leave3Npc = self:NPC_createNormal('逃離沙漏', 235179, { map = 60007, x = 18, y = 32, direction = 6, mapType = 0 })
  self:NPC_regWindowTalkedEvent(Leave3Npc, function(npc, player, _seqno, _select, _data)
  end)
  self:NPC_regTalkedEvent(Leave3Npc, function(npc, player)
	if (NLG.CanTalk(npc, player) == true) then
	...
	for k,v in pairs(BossRoom) do
		if (Char.ItemNum(player, v.keyItem)>0)  then
			...
			if (Num>=v.posNum_L and Num<v.posNum_R and Rank==v.bossRank)then
				...
				Battle.SetWinEvent("./lua/Modules/mazeBoss3.lua", "def_round_wincallback", battleindex);
			end
		end
	end
end
...
function Module:OnbattleStartEventCallback(battleIndex)
	local ret = SQL.Run("select Name,WorldLord3 from lua_hook_worldboss order by WorldLord3 asc limit 3");
	if(type(ret)=="table" and ret["0_1"]~=nil)then
		LordHP3=tonumber(ret["0_1"]);
	end
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		local HP = LordHP3;
		if enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
...
function Module:OnBeforeBattleTurnCommand(battleIndex)
	...
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		local HP = LordHP3;
		if Round==0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
		elseif Round>0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
			if Round>=5 then
			...
			if NLG.Rand(1,10)>=3  then
				local BuffData = {CONST.CHAR_BattleDamageVanish, CONST.CHAR_BattleDamageMagicVanish};
				local ch = NLG.Rand(1,2);
				Char.SetData(enemy, BuffData[ch], 4);    --仅能挡一下攻击
			end
			NLG.UpChar(enemy);
...
function Module:OnAfterBattleTurnCommand(battleIndex)
	...
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		if Round>=0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
			local HP_Less = Char.GetData(enemy,CONST.CHAR_血);  --註解後新加
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);
			Char.SetData(enemy, CONST.CHAR_血, HP_Less);
			NLG.SystemMessage(player,"[系統]殉情特性發動，區域領主目前剩餘血量"..HP_Less.."！");
			NLG.UpChar(enemy);
			--Lord血量写入库
			...
			if (cdk~=nil) then
				...
				SQL.Run("update lua_hook_worldboss set WorldLord3= '"..HP_Less.."' where CdKey='"..cdk.."'")
				NLG.UpChar(player);
			end
		end
	end
...
function Module:OnEnemyCommandCallBack(battleIndex, side, slot, action)
      ...
      for i = 10, 19 do
            local enemy = Battle.GetPlayer(battleIndex, i);
            if Round>=10 and Round<=14 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_DEATH, 40, 8609);
            elseif Round>=15 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406192  then
