...
--     十(9)	八(7)	六(5)	七(6)	九(8)
------------对战NPC设置------------
EnemySet[1] = {0, 406221, 406200, 406222, 406201, 0, 406223, 406202, 406224, 406203}    --0代表没有怪
EnemySet[2] = {0, 0, 0, 0, 0, 0, 406225, 406204, 0, 0}
EnemySet[3] = {0, 0, 0, 0, 0, 0, 406226, 406206, 0, 0}
BaseLevelSet[1] = {0, 150, 150, 150, 150, 0, 150, 150, 150, 150}
BaseLevelSet[2] = {0, 0, 0, 0, 0, 0, 160, 160, 0, 0}
BaseLevelSet[3] = {0, 0, 0, 0, 0, 0, 160, 160, 0, 0}
Pos[1] = {"洞窟魔鴨",EnemySet[1],BaseLevelSet[1]}
Pos[2] = {"暴走霸王",EnemySet[2],BaseLevelSet[2]}
Pos[3] = {"暴走霸王",EnemySet[3],BaseLevelSet[3]}

------------------------------------------------
--背景设置
local Switch = 1;                          --组队人数限制开关1开0关
local Rank = 0;                             --初始化难度分类
local BossMap= {60013,22,14} -- 战斗场景Floor,X,Y(弱、普、超同场景)
...
local BossRoom = {
      { key=1, keyItem=70195, keyItem_count=1, bossRank=1, limit=-1, posNum_L=1, posNum_R=2,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70267, getItem_count = 1},
          lordName="洞窟魔鴨",
       },    -- 虚弱(1)
      { key=3, keyItem=70195, keyItem_count=1, bossRank=2, limit=3, posNum_L=2, posNum_R=3,
          win={warpWMap=60001, warpWX=21, warpWY=30, getItem = 70267, getItem_count = 6},
          lordName="洞窟魔鴨",
       },    -- 普通(2)
...
function Module:onLoad()
  ...
  local Lord6Npc = self:NPC_createNormal('區域領主討伐', 11394, { map = 60012, x = 64, y = 43, direction = 6, mapType = 0 })
  ...
  self:NPC_regWindowTalkedEvent(Lord6Npc, function(npc, player, _seqno, _select, _data)
	...
	if seqno == 1 then
		if data == 1 then  ----参加领主讨伐
			local retEnd = SQL.Run("select Name,LordEnd6 from lua_hook_worldboss order by LordEnd6 desc ");
			...
			if(Char.ItemNum(player,BossKey[1])>0 or Char.ItemNum(player,BossKey[2])>0 or Char.ItemNum(player,BossKey[3])>0) then
				...
				return;
			elseif (Char.ItemNum(player,16442)<=0) then
				NLG.SystemMessage(player,"[系統]此處刻著暴食印記，似乎須要暴食的罪書來共鳴。");
				return;
			else
		...
		if key == data+4 then
			...
			for k,v in pairs(BossRoom) do
				if ( key==v.key ) then
					...
					Char.SetLoopEvent('./lua/Modules/mazeBoss6.lua','AutoLord_LoopEvent',player,1000);
					--WorldDate = {}
					WorldDate[6] = {
					os.date("%w",os.time()),
					}
  ...
  local Leave6Npc = self:NPC_createNormal('逃離沙漏', 235179, { map = 60013, x = 14, y = 25, direction = 6, mapType = 0 })
  self:NPC_regWindowTalkedEvent(Leave6Npc, function(npc, player, _seqno, _select, _data)
  end)
  self:NPC_regTalkedEvent(Leave6Npc, function(npc, player)
	if (NLG.CanTalk(npc, player) == true) then
...
function Module:OnbattleStartEventCallback(battleIndex)
	local Sum=0;
	local ret = SQL.Run("select Name,WorldLord6 from lua_hook_worldboss order by WorldLord6 asc limit 3");
	...
	for i = 10, 19 do
		...
		local HP = LordHP6;
		if enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406206  then
...
function Module:OnBeforeBattleTurnCommand(battleIndex)
	...
	for i = 10, 19 do
		local enemy = Battle.GetPlayer(battleIndex, i);
		local HP = LordHP6;
		if Round==0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406206  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
			Char.SetData(enemy, CONST.CHAR_防御力, 420);
		elseif Round>0 and enemy>=0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406206  then
			Char.SetData(enemy, CONST.CHAR_最大血, 1000000);     --血量上限100万
			Char.SetData(enemy, CONST.CHAR_血, HP);
			Char.SetData(enemy, CONST.CHAR_防御力, 420);
			if Round>=5 then
			...
			if NLG.Rand(1,10)>=7  then
				local BuffData = {CONST.CHAR_BattleModPoison};
...
function Module:OnEnemyCommandCallBack(battleIndex, side, slot, action)
      ...
      for i = 10, 19 do
            local enemy = Battle.GetPlayer(battleIndex, i);
            if Round>=3 and Round<=4 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406226  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_BOMB, 40, 8200);
            elseif Round>=10 and Round<=14 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406206  then
                          SetCom(enemy, action, CONST.BATTLE_COM.BATTLE_COM_M_DEATH, 40, 8609);
            elseif Round>=15 and enemy>= 0 and Char.GetData(enemy, CONST.对象_ENEMY_ID)==406206  then
