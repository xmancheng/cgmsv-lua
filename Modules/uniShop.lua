...
UniList[sss] = {}
UniList[sss][1] = {
{ name = '道具上架许可印章', image = 26569, price = 0, desc = '物品尚未上架x请勿购买', count = 1, maxCount = 1, itemid = -1, gold = 0 },
{ name = '道具上架许可印章', image = 26569, price = 0, desc = '物品尚未上架x请勿购买', count = 1, maxCount = 1, itemid = -1, gold = 0 },
{ name = '道具上架许可印章', image = 26569, price = 0, desc = '物品尚未上架x请勿购买', count = 1, maxCount = 1, itemid = -1, gold = 0 },
{ name = '道具上架许可印章', image = 26569, price = 0, desc = '物品尚未上架x请勿购买', count = 1, maxCount = 1, itemid = -1, gold = 0 },
{ name = '道具上架许可印章', image = 26569, price = 0, desc = '物品尚未上架x请勿购买', count = 1, maxCount = 1, itemid = -1, gold = 0 },
...
function UniShop:onLoad()
  self:logInfo('load')
  local uninpc = self:NPC_createNormal('镜像摆摊租借', 231051, { map = 1000, x = 223, y = 109, direction = 4, mapType = 0 })
     ...
     if data == 2 then  ----删除镜像摆摊&执行
       ...
       ----领回寄售道具 (同功能5)
            for k = 1, 5 do
                ...
                local maxCount = 0;
                if (maxCount ==0) then
                     UniList[Number][1][k].name = '道具上架许可印章';
                     UniList[Number][1][k].image = 26569;
                     UniList[Number][1][k].price = 0;
                     UniList[Number][1][k].desc = '物品尚未上架x请勿购买';
...
function UniShop:onSellerTalked(npc, player)  ----设立镜像摊位
  if (NLG.CanTalk(npc, player) == true) then
    ...
    NLG.ShowWindowTalked(player, npc, CONST.窗口_商店买, CONST.BUTTON_是, 0,
      self:NPC_buildBuyWindowData(npcSkin, '镜像摆摊', '贩卖商品', '金钱不足', '背包已满', UniList[Number][1]))
  end
end
...
function UniShop:onSellerSelected(npc, player, seqNo, select, data)
  ...
  if maxCount < 0 then
    NLG.SystemMessage(player,"[系統]数量超过库存！");
    return;
  end
