...
local BossKey= {70213,70213,70213} -- 虛弱、普通、超級
local Pts= 69000;                        --积分券
local BossRoom = {
		...
		if data == 3 then  ----观看讨伐实况
			...
			local msg = "3\\n@c觀戰目前討伐的實況\\n"
				...
				if (duelplayerName~=nil and Rank>=1) then
					msg = msg .. "領主挑戰者:" ..duelplayerName.. "  ★".. rankLevel[Rank] .."模式\\n"
				end
			end
	...
	for k,v in pairs(BossRoom) do
		if (Char.ItemNum(player, v.keyItem)>0)  then
			...
			if (Num>=v.posNum_L and Num<v.posNum_R and Rank==v.bossRank)then
				Char.HealAll(player);
				SetEnemySet(Rank);
				NLG.SystemMessage(-1,"領主挑戰者:"..Char.GetData(player,%对象_名字%));
